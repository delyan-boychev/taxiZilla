    ...
    getRequest("/order/getOrderMessage").then(data=>
        {
            if(data["isAccepted"] == true)
            {
                ...
                if(orderType == "taxi")
                {
                    text = `До няколко минути ще дойде <span class="font-weight-bold">${data["driverName"]}</span>, който се намира най-близо до Вас! Благодарим Ви, че използвате taxiZilla и приятно пътуване!`;
                }
                else if(orderType == "items")
                {
                    text=`${data["driverName"]} ще изпълни Вашата поръчка за пазаруване. Скоро очаквайте обаждане от шофьора! Благодарим Ви, че използвате taxiZilla!`;
                }
                if(data["driverName"]=="")
                {
